...
[Tie] Pimeys' version has been enhanced in this fork:
. A background daemon sends the notification from a different process
. [TBD] HTTP keep-alive and pipeline experiments
...
To run the daemon:
<pre><code> $ cd RAILS_ROOT; c2dms <RAILS_ENV> </code></pre>

To run the daemon in the foreground:
<pre><code> $ cd RAILS_ROOT; c2dms <RAILS_ENV> --foreground </code></pre>
